...
class CloneService:
    def __init__(self, command_runner: CommandRunner | None = None, timeout_seconds: int = 300) -> None:
    ...
    def checkout_pull_request(self, metadata: PRMetadata, run_dir: str | Path) -> CheckoutOutcome:
        ...
        base_result = self.command_runner.run(fetch_base, timeout_seconds=self.timeout_seconds)
        ...
        head_result = self.command_runner.run(fetch_head, timeout_seconds=self.timeout_seconds)
        ...
        checkout_result = self.command_runner.run(checkout, timeout_seconds=self.timeout_seconds)
